...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Start()
    {
        content = GameObject.Find("Canvas").transform.GetChild(5).GetChild(0).gameObject;
        hpUI = content.GetComponent<RectTransform>();
    ...
    void Update()
    {
        ...
        if (controller.isStart == true)
        {
            ...
            {
                GameObject newBullet = Instantiate(theBullet, firPos.position, firPos.rotation);
            }
        }
    }
    float a =0;
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag == "Enemy")
        {
            ...
            if(HP<=0)
            {
                ...
                controller.failPanel.SetActive(true);
            }
        }
        if (other.tag == "box")
        {
